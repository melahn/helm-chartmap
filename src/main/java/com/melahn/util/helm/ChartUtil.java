...
public final class ChartUtil {
    ...
    @SuppressWarnings("unchecked") public static Object getValue(String k, Map<String, Object> values) {
        if (values != null) {
            String head = getLead(k);
            String tail = getTail(k);
        if (!head.isEmpty()) {
     ...
     * @return returns a String with the leading segment of the string up but
     * not including the dot.  If the string does not contains a dot returns the
     * input string.
     */
    private static String getLead(String k) {
        if ((k.contains("."))) {
     ...
     * 
     * @param k the string from which the extraction should occur
     * @return returns a String with the trailing segment of the string starting with 
     * the position just past the dot.  If the string does not contains a dot returns the
     * empty string.
     */
